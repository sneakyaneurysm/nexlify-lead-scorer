...
### Executive Summary
AI-Powered Lead Scoring Dashboard prioritizes leads in Nexlify’s CRM using a simple, transparent model based on interaction data. It integrates with existing services and MongoDB, and ships as a React + Tailwind UI on Vercel with an Express-compatible API.
...
- Frontend: React (Next.js recommended) on Vercel, Tailwind CSS.
- Backend: Express-compatible serverless API route /leads/scoring.
...
### Scope
- In scope: React dashboard (sortable table); GET /leads/scoring; weighted-sum scoring; MongoDB read/write; Vercel deploy; Tailwind.
...
- React + Tailwind on Vercel; SWR for fetching/caching and auto-refresh.
...
- Hosting: Vercel (Next.js app + /api/leads/scoring serverless function).
...
- Secrets in Vercel; local dev via .env.local; Atlas networking via allowlist/peering.
...
- 25–30m: Configure Vercel env; deploy; manual smoke.
